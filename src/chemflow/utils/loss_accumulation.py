...
class LossAccumulator:
    ...
    def log_dict(self) -> dict[str, torch.Tensor | float]:
        ...
        for key in self._raw_losses.keys():
            pure_val = self._raw_losses[key].detach().cpu()

            # Use current_weights here
            weight_k = current_weights.get(key, 1.0)
            fully_weighted_val = (weight_k * self._tw_losses[key]).detach().cpu()
        ...
        for group, keys in self._groups.items():
            pure_terms = [
                self._raw_losses[k].detach().cpu()
                for k in keys
                if k in self._raw_losses
            ]
            weighted_terms = [
                (current_weights.get(k, 1.0) * self._tw_losses[k]).detach().cpu()
                for k in keys
