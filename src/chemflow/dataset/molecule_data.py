...
def filter_nodes(
    ...
    if isinstance(mol, MoleculeBatch):
        ...
        if getattr(mol, "scaffold_mask", None) is not None:
            result.scaffold_mask = mol.scaffold_mask[mask]
        if getattr(mol, "target_props", None) is not None:
            result.target_props = mol.target_props
        return result
...
def sort_nodes_by_batch(data):
    ...
    if getattr(data, "scaffold_mask", None) is not None:
