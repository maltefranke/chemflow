...
Run from repository root::
    ...
    python -m chemflow.rl.experiments.shape.generate_shape_grpo_samples

Or::

    python src/chemflow/rl/experiments/shape/generate_shape_grpo_samples.py

Writes ``shape_grpo_samples.pt`` under ``<out_dir>/<rl_ckpt_stem>/`` (same layout idea as
``chemflow.rl.experiments.natoms.compare_pretrained_rl_atoms``). The active reward registry no
...
def _gather_predict(module, datamodule, cfg, ckpt_path: str, n_mols: int, max_atoms: int):
    ...
    # Default Hydra uses ddp + bf16 (cluster). Safe defaults for local / notebook-style runs.
...
def main(default_rl_ckpt_name: str = "grpo_shape_seed0_sig0p05_g1_mu2_kl0.05_lr1e-4_maxa60_omitposkl_best.pt"):
    ...
    ap.add_argument("--max_atoms", type=int, default=60, help="integrator.max_atoms after load (GRPO training uses 60).")
    ...
    ap.add_argument(
        ...
        help="Base directory; artifact goes to <out_dir>/<rl_ckpt_stem>/shape_grpo_samples.pt",
    )
    ap.add_argument(
        "--out",
        default=None,
    ...
    ap.add_argument(
        "--no-save-invalid",
        action="store_true",
        help="Omit invalid trajectories from the file (smaller; invalid counts unavailable in notebook).",
    )
    ap.add_argument(
        "overrides",
        nargs="*",
    ...
    out_path = args.out or os.path.join(run_dir, "shape_grpo_samples.pt")
    ...
    module_base, valid_base, inv_base = _gather_predict(
        module_base, dm_base, cfg, args.pretrained_ckpt, args.n_mols, args.max_atoms
    )
    ...
    module_rl, valid_rl, inv_rl = _gather_predict(
        module_rl, dm_rl, cfg, rl_ckpt, args.n_mols, args.max_atoms
    )
    vocab_rl = _vocab_dict(module_rl)
    if vocab_base != vocab_rl:
        print("warning: vocab differs between base and RL modules; saved file uses RL vocab for `vocab` key.")
