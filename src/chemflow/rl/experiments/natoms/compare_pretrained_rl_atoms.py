...
Run from repository root::
    ...
    python -m chemflow.rl.experiments.natoms.compare_pretrained_rl_atoms

Or::

    python src/chemflow/rl/experiments/natoms/compare_pretrained_rl_atoms.py

Uses the same Hydra stack and checkpoint loading as ``chemflow.rl.run_grpo``.
After each load we set ``module.integrator.max_atoms`` (default ``60``, same as ``chemflow.rl.run_grpo``)
...
``chemflow.rl.rewards.simple.n_atoms_reward``. This includes explicit hydrogens if ``"H"`` is in
