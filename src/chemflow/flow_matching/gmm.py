...
def sample_from_typed_gmm(means, types, t, N_types, sigma=1.0, num_samples=100):
    ...
    # Define the "noise" distribution (p0) as uniform
...
def typed_gmm_loss(
    ...
    if target_locations.dim() == 2:
        ...
        if target_types.dim() == 1:
            # If target_types is (N_samples,), add batch dimension
    ...
    # Broadcasting [B, N_samples, 1] with [B, 1, K] gives [B, N_samples, K]
    # Resulting shape: [B, N_samples, K]
    log_prob_types = type_dist.log_prob(target_types.unsqueeze(2))
