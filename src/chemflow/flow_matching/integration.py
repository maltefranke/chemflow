...
from chemflow.flow_matching.gmm import sample_from_typed_gmm
...
def sample_death_process_gnn(
    ...
    Args:
        ...
        ct: Shape (N_total, num_classes) - class predictions (logits)
    ...
    death_mask = torch.zeros_like(xt_mask, dtype=torch.bool, device=device)

    # Compute probabilities from class predictions
    # ct is (N_total, num_classes), get softmax probabilities
    probs = F.softmax(ct, dim=-1)  # Shape: (N_total, num_classes)

    # Extract probability for death token for each node
    death_token_probs = probs[:, death_token_index]  # Shape: (N_total,)
    ...
    for graph_id in range(num_graphs):
        ...
        if num_valid > 0 and num_deaths_g > 0:
            ...
            _, death_indices = torch.topk(graph_death_probs, num_to_kill, largest=True)
...
def sample_birth_process_gnn(
    ...
    for graph_id in range(num_graphs):
        ...
        sampled_locations, sampled_types = sample_from_typed_gmm(
            gmm_params, num_births, K, D, N_types
        )
        ...
        new_batch_ids_list.append(
            torch.full((num_births,), graph_id, device=device, dtype=batch_id.dtype)
        )

    if len(new_particles_list) == 0:
        # No births, return empty tensors
        empty_particles = torch.empty((0, D), device=device)
        empty_types = torch.empty((0, N_types), device=device)
        empty_batch_ids = torch.empty((0,), device=device, dtype=batch_id.dtype)
...
def integrate_step_gnn(
    ...
    node_times = torch.zeros_like(batch_id, dtype=t.dtype, device=device)
    for graph_id in range(num_graphs):
    ...
    death_mask = sample_death_process_gnn(
        global_death_rate, ct, xt_mask, batch_id, death_token_index, dt, device
    )
