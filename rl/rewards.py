...
def topology_reward(module, trajectory) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    Components:
      - ring-aware element/order MCS, normalized by the larger molecule
      - Murcko scaffold RDK-fingerprint similarity
      - 50/50 RDK path + Morgan circular fingerprint blend
      - Gaussian heavy-atom size regularization
    """
    device = trajectory.mol_final.x.device
    rewards: list[float] = []
    raw_vals: list[float] = []
